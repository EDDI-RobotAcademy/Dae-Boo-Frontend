<template>
  ...
  <v-menu>
    <template v-slot:activator="{ props }">
      <v-col cols="auto">
      <v-btn v-bind="props" v-if="!user.hasOwnProperty('email')" rounded variant="plain" style="color: rgb(255, 255, 255)"  icon="mdi-account"></v-btn>
        <v-btn v-bind="props" v-if="user.hasOwnProperty('email')" rounded variant="plain" style="color: rgb(255, 255, 255)" @click="signOut"></v-btn>
      </v-col>
    </template>
